...
def print_preflight(config_path: str | Path, config: ExperimentConfig, samples: list, *, cache_root: Path | None = None) -> None:
    print(f"[preflight] config={config_path}")
    print(f"[preflight] output_root={config.output_root}")
    if cache_root is not None:
        print(f"[preflight] roi_cache_root={cache_root}")
...
def build_roi_cache_from_config(
    ...
    *,
    ...
    print_preflight(config_path, config, samples, cache_root=cache_root)
    if dry_run:
        return None
